...
PARAM  <- list()
PARAM$experimento  <- "HT7510_002"

PARAM$exp_input  <- "TS7410_002"
...
exp_log  <- function( reg, arch=NA, folder="./exp002/", ext=".txt", verbose=TRUE )
{
  archivo  <- arch
...
dataset_input  <- paste0( "./exp002/", PARAM$exp_input, "/dataset_training.csv.gz" )
...
dir.create( paste0( "./exp002/", PARAM$experimento, "/"), showWarnings = FALSE )
setwd(paste0( "./exp002/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO

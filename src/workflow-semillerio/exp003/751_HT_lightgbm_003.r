...
PARAM  <- list()
PARAM$experimento  <- "HT7510_003"

PARAM$exp_input  <- "TS7410_003"
...
exp_log  <- function( reg, arch=NA, folder="./exp003/", ext=".txt", verbose=TRUE )
{
  archivo  <- arch
...
dataset_input  <- paste0( "./exp003/", PARAM$exp_input, "/dataset_training.csv.gz" )
...
dir.create( paste0( "./exp003/", PARAM$experimento, "/"), showWarnings = FALSE )
setwd(paste0( "./exp003/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO

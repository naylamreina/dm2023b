...
PARAM  <- list()
PARAM$experimento <- "FE7310_012"

PARAM$exp_input  <- "DR7210_012"
...
PARAM$CanaritosAsesinos$semilla  <- 323591   # cambiar por la propia semilla
...
CanaritosAsesinos  <- function( canaritos_ratio=0.2, canaritos_desvios=3.0, canaritos_semilla=999983 )
{
  ...
  param <- list( objective= "binary",
                 ...
                 min_gain_to_split= 0.001, #por ahora, lo dejo fijo
                 lambda_l1= 0.01,         #por ahora, lo dejo fijo
                 lambda_l2= 0.01,         #por ahora, lo dejo fijo
                 max_bin= 80,            #por ahora, lo dejo fijo
...
dataset_input  <- paste0( "./exp012/", PARAM$exp_input, "/dataset.csv.gz" )
...
dir.create( paste0( "./exp012/", PARAM$experimento, "/"), showWarnings = FALSE )
setwd(paste0( "./exp012/", PARAM$experimento, "/"))   #Establezco el Working Directory DEL EXPERIMENTO
